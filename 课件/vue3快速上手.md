...
- 实现原理：
  ...
  - 数组类型：通过重写更新数组的一系列方法来实现拦截。（对数组的变更方法进行了包裹二次封装）。
...
- shallowReadonly：让一个响应式数据变为只读的（浅只读），深层次的数据可以修改。
- 应用场景: 不希望数据被修改时。
...
- <strong style="color:#DD5145">移除</strong>```v-on.native```修饰符，子组件中没有声明，默认为原生事件
